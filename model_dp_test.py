...
class network(nn.Module):
    def __init__(self, config=None, arch=None, seqlen=4096, debug=False, dpblock_flag=True, replace=False):
        ...
        self.layer_dict = {12: 768, 0 : 256, 3: 320, 4: 384, 5: 448, 6: 512, 16: 768}
        if debug:
        ...
        ##################################################################################################
        dplayer = config.layer
        for i, layer in enumerate(arch):
            print(layer)
            if i >= dplayer: # insert the dynamic pooling and get in advance of the dynamic pooling.
                i = i-1
            ...
            if len(layer) > 7 and i == dplayer:  # if the layer is dynamic pooling
...
if __name__ == "__main__":
    ...
    if continue_training:
        args.statedict = 'runs-epoch17.torch'
    if args.name == None:
        args.name = "training"
        while args.name == "":
